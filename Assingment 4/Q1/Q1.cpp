...
public:
    ...
    void updateAccount(float amount){
        if(balance<(-amount)){
            cout<<"\nNot Enough alance. Money cant e withdrawed.\n";
            return;
        }
        balance+=amount;
    }
...
ACCOUNT* ACCOUNT_LIST::searchAccount(string accountNumer){
        for(int i=0;i<top;i++){
...
public:
    WITHDRAW(string accountNumber,float withdrawAmmount){
        ...
        ACCOUNT* a=(l.searchAccount(accountNumber));
    ...
    while(option!=-1){
        ...
        case 1:
           { string accNum; 
            ...
            if(l.searchAccount(accNum)!=NULL){
